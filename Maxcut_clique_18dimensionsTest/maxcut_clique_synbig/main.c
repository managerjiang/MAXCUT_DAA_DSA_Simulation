...
#include "time.h"
#include <math.h>
#define AGENT 6   ////并行个体数个数-n 6，实际agent 5，0号用于分发数据，收集数据
#define DIM 18
...
void main(int argc, char* argv[]) {
    ...
    for (int ite = 0;ite < iterations;ite++)
    {
        if ((myrank == 4) || (myrank == 5))
        {
            ...
            if (myrank == 5)
                tag = 52;
             MPI_Ssend(send_v, DIM, MPI_DOUBLE, 2, tag, MPI_COMM_WORLD);///向上传信息
        ...
        if (myrank == 2)
        {
            ...
            int tag = 21;
             ...
             MPI_Ssend(send_v, DIM, MPI_DOUBLE, 1, tag, MPI_COMM_WORLD);///向上传信息
             MPI_Ssend(send_v2, DIM, MPI_DOUBLE, 1, 22, MPI_COMM_WORLD);///向上传信息*/
            ...
            for (i = 0;i < 6;i++)
            {
                ...
                if ((mm != -1) && (mm != 0) && (mm != 3))
                {
                    ...
                    if (mm == 2)
                    {
                        ...
                        MPI_Ssend(send_rl, DIM, MPI_DOUBLE, 4, 24, MPI_COMM_WORLD);///向下传信息
                        MPI_Ssend(send_rl, DIM, MPI_DOUBLE, 5, 25, MPI_COMM_WORLD);///向下传信息
                    ...
                    if (mm == 12)
                    {
                        ...
                        MPI_Ssend(send_rl, DIM, MPI_DOUBLE, 4, 34, MPI_COMM_WORLD);///向下传信息
                        MPI_Ssend(send_rl, DIM, MPI_DOUBLE, 5, 35, MPI_COMM_WORLD);///向下传信息
            ...
            double v_recvp[DIM];
            double v_recvp2[DIM];
            MPI_Recv(v_recvp, DIM, MPI_DOUBLE, 1, 11, MPI_COMM_WORLD, &status_v2[0]);//接受来自父的信息
        ...
        if (myrank == 3)
        {
            ...
            for (i = 0;i < 6;i++)
            {
             ...
             MPI_Ssend(send_v, DIM, MPI_DOUBLE, 1, 31, MPI_COMM_WORLD);///向上传信息
            ...
            for (i = 0;i < 6;i++)
            {
             ...
             MPI_Ssend(send_v2, DIM, MPI_DOUBLE, 1, 32, MPI_COMM_WORLD);///向上传信息
            ...
            double v_recv[DIM];
            int tag_r = 13;
            MPI_Recv(v_recv, DIM, MPI_DOUBLE, 1, tag_r, MPI_COMM_WORLD, &status_v[0]);//接受来自父信息
            ...
            double v_recv2[DIM];
             tag_r = 14;
            MPI_Recv(v_recv2, DIM, MPI_DOUBLE, 1, 14, MPI_COMM_WORLD, &status_v[1]);//接受来自父信息
            ...
            for (i = 0;i < 6;i++)
            {
                ...
                if ((mm != -1))
                {
                    ...
                    if (mm == 0)
                    {
                        for (j = 0;j < DIM;j++)
                            send_rl[j] = V_variable[j][mm];
                        MPI_Ssend(send_rl, DIM, MPI_DOUBLE, 2, 11, MPI_COMM_WORLD);///向下传信息
                    ...
                    if (mm == 3)
                    {
                        for (j = 0;j < DIM;j++)
                         ...
                         MPI_Ssend(send_rl4, DIM, MPI_DOUBLE, 2, 12, MPI_COMM_WORLD);///向下传信息
                         MPI_Ssend(send_rl4, DIM, MPI_DOUBLE, 3, 13, MPI_COMM_WORLD);///向下*/
                    ...
                    if (mm == 13)
                    {
                        ...
                        MPI_Ssend(sendrl2, DIM, MPI_DOUBLE, 3, 14, MPI_COMM_WORLD);///向下传信息
